...
(async function () {
  ...
  const loadVueComponent = async function (file) {
    ...
    var components = ['filter-card', 'search-result', 'left-panel', 'right-panel', 'multiple-checkbox-filter', 'has-or-missing-filter', 'boolean-radio-filter', 'date-range-filter', 'search-top', 'restaurant-modal'];
  ...
  new Vue({
    ...
    data: {
      variants: {
        ...
        "info": { action: "loadFilters", status: 0, title: 'Loading filter left sidebar' },
    ...
    methods: {
      ...
      async loadVueComponent() {
        ...
        await Promise.all([
          ...
          // loadJsComponent('button-counted'),
        ]);
